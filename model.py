...
class BNPReLU(nn.Module):  # 激活函数与归一化
...
class DABModule(nn.Module):
	def __init__(self, nIn, d=1, kSize=3, dkSize=3):
		...
		self.dconv3x1 = nn.Sequential(
			Conv(nIn // 2, nIn // 2, (dkSize, 1), 1,
				 padding=(1, 0), groups=nIn // 2, bn_acti=True),
			CBAMBlock(channel=nIn // 2, reduction=4),
		)
		
		self.dconv1x3 = nn.Sequential(
			Conv(nIn // 2, nIn // 2, (1, dkSize), 1,
				 padding=(0, 1), groups=nIn // 2, bn_acti=True),
			CBAMBlock(channel=nIn // 2, reduction=4),
		)
		
		self.ddconv3x1 = nn.Sequential(
			Conv(nIn // 2, nIn // 2, (dkSize, 1), 1,
				 padding=(1 * d, 0), dilation=(d, 1), groups=nIn // 2, bn_acti=True),
			CBAMBlock(channel=nIn // 2, reduction=4),
		)
		
		self.ddconv1x3 = nn.Sequential(
			Conv(nIn // 2, nIn // 2, (1, dkSize), 1,
		...
		self.conv1x1 = nn.Sequential(
			Conv(nIn // 2, nIn, 1, 1, padding=0, bn_acti=False),
			CBAMBlock(channel=nIn, reduction=4),
		)
...
class InputInjection(nn.Module):  # 池化层
...
class DABNet(nn.Module):
	def __init__(self, classes=4, block_1=3, block_2=6):
		super().__init__()
		self.init_conv = nn.Sequential(
			Conv(3, 32, 3, 2, padding=1, bn_acti=True),
			Conv(32, 32, 3, 1, padding=1, bn_acti=True),
			Conv(32, 32, 3, 1, padding=1, bn_acti=True),
			CBAMBlock(channel=32, reduction=8),
		)
